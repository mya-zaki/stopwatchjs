var stopWatch = (function () {
    ...
    var stopWatch = function (options) {
        var sw = new stopWatch.prototype.init(options);
    ...
    stopWatch.getStatus = function (name) {
        var sw = new stopWatch.prototype.init({
            name: name || null
        });
        return sw.getStatus();
    };
    ...
    stopWatch.prototype = {
        ...
        init: function (options) {
            ...
            this.render = function () {
                ...
                render(this.getStrHour(), this.getStrMin(), this.getStrSec(), this.getStrSec100th());
            };
        ...
        getCurrent: function () {
            return (this.hour * 60 * 60 * 1000 + this.min * 60 * 1000 + this.sec * 1000 + this.millisec);
        },
        ...
        store: function () {
            try {
                localStorage.setItem('sw-' + this.name, btoa(JSON.stringify({
        ...
        restore: function () {
            try {
                var itemtext = localStorage.getItem('sw-' + this.name);
            } catch(e) {
                console.error(e);
            }
        ...
        getStrHour: function () {
            var strHour = this.hour.toString();
            if (strHour.length < 2) {
                strHour = "0" + strHour;
            }
            return strHour;
        ...
        getStrSec100th: function () {
            var strSec100th = this.sec100th.toString();
